...
class Client(base.Client):
    def __init__(self, *args, **kwargs):
        ...
        if 'ssh_' in grouped_kwargs:
            ...
            self.ssh_shell = Shell(self.ssh_client, shell_prompt, optional_args=shell_args)
    ...
    def software_version(self):
        if self.can_snmp():
            return self._asa_version(self.snmp_client.sysDescr())
        elif self.can_ssh():
            return self._asa_version(self.ssh_shell.command('show version')[1])
        return ''
    ...
    @staticmethod
    def _asa_version(version_string):
        re_match = re.match(r'Cisco Adaptive Security Appliance Software Version ([^\,]+) ', version_string)
